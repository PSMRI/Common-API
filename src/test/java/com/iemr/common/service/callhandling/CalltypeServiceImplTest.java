...
class CalltypeServiceImplTest {
    ...
    void getAllCalltypes_inboundOutbound_success() throws Exception {
        ...
        when(callTypeMapper.fromJson(anyString(), eq(CallType.class))).thenReturn(inboundOutboundProvider);
    ...
    void getAllCalltypes_default_success() throws Exception {
        ...
        when(callTypeMapper.fromJson(anyString(), eq(CallType.class))).thenReturn(defaultProvider);
    ...
    void getAllCalltypesV1_emptyResult() throws Exception {
        String json = "{\"providerServiceMapID\":4}";
        // A payload that carries neither direction flag falls back to the provider-only lookup.
